...
with st.spinner("正在加载基金列表..."):
    fund_df = cached_fund_list(fund_type)

if fund_df.empty:
    st.error("❌ 无法获取基金列表，请检查网络后刷新。")
    st.stop()

st.success(f"✅ 已加载 **{len(fund_df)}** 只基金")
...
# Step 2: 获取净值并计算风险指标
start_str = date_range[0].strftime("%Y%m%d") if isinstance(date_range, tuple) else date_range[0].strftime("%Y%m%d")
end_str = date_range[1].strftime("%Y%m%d") if isinstance(date_range, tuple) else date_range[1].strftime("%Y%m%d")

if st.button("🚀 开始分析", type="primary", use_container_width=True):
    funds_nav = {}
    funds_daily = {}  # 保存完整日数据供明细表使用
    progress = st.progress(0, text="正在获取净值数据...")

    failed = []
    for i, (code, name) in enumerate(zip(selected_codes, selected_names)):
        progress.progress((i) / len(selected_codes), text=f"获取 {name} ({code})...")
