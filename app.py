...
if st.button("🚀 开始分析", type="primary", use_container_width=True):
    ...
    st.divider()
    st.subheader("📊 风险指标总览")
    st.caption("绿色 = 该指标在同组中表现优秀 · 红色 = 表现较差")
...
| 指标 | 说明 | 好坏判断 |
|------|------|----------|
| **年化收益(%)** | 按复利折算的年均收益率 | 越高越好 |
| **年化波动率(%)** | 日收益率的标准差年化值，反映价格波动幅度 | 越低越稳 |
| **最大回撤(%)** | 选定区间内从最高点到最低点的最大跌幅 | 越小越好 |
| **夏普比率** | (年化收益-无风险利率)÷年化波动率，衡量风险调整后收益 | >1 不错，>2 优秀 |
| **索提诺比率** | 类似夏普，但只计算下行波动率 | 越高越好 |
| **卡玛比率** | 年化收益÷最大回撤，衡量每承受1%回撤能获得多少收益 | 越高越"划算" |
| **VaR 95%(%)** | 95%置信度下，日最大亏损不超过该值 | 越小越好 |
| **CVaR 95%(%)** | 超出VaR后的平均亏损，衡量尾部风险 | 越小越好 |
| **日胜率(%)** | 交易日中上涨的比例 | 越高越好 |
| **样本天数** | 用于计算的有效交易日数 | ≥60为宜 |
        """)
